 ...
 * Licensed under MIT (https://github.com/twbs/cranberry/blob/master/LICENSE)
 */
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    jqueryCheck: 'if (typeof jQuery === \'undefined\') {\n' +
                 '  throw new Error(\'cranberry\\\'s JavaScript requires jQuery\')\n' +
                 '}\n',
    jqueryVersionCheck: '+function ($) {\n' +
                        '  var version = $.fn.jquery.split(\' \')[0].split(\'.\')\n' +
                        '  if ((version[0] < 2 && version[1] < 9) || (version[0] == 1 && version[1] == 9 && version[2] < 1) || (version[0] >= 4)) {\n' +
                        '    throw new Error(\'cranberry\\\'s JavaScript requires at least jQuery v1.9.1 but less than v4.0.0\')\n' +
                        '  }\n' +
                        '}(jQuery);\n\n',
    ...
    /* Cranberry: we do not have any js compilation for now
    ...
    babel: {
      ...
      dist: {
        ...
        files: {
          '<%= concat.cranberry.dest %>' : '<%= concat.cranberry.dest %>'
        }
      }
    },
    */
    stamp: {
      ...
      cranberry: {
        files: {
          src: '<%= concat.cranberry.dest %>'
        }
      }
    },
// Cranberry : We do not need to concat now
  //  concat: {
  //    options: {
  ...
  //      src: [
   ...
   // },
//
// Cranberry: We do not need any js build for now
  /*  uglify: {
      options: {
        compress: {
          warnings: false
...
/*  Cranberry: Guessing that jekyll is used for docs here, so removing it. As we do not need documentation now
    jekyll: {
      options: {
        bundleExec: true,
        config: '_config.yml',
 ...
 /* Cranberry: We do not need HTML lint for now
   htmllint: {
      options: {
        ignore: [
...
*/ 
 // Cranberry: We do not need watch for now
 //   watch: {
 //     src: {
 //       files: '<%= concat.cranberry.src %>',
 //       tasks: ['babel:dev']
 //     },
 //     sass: {
 //       files: 'scss/**/*.scss',
 ...
 //     }
 //   },
  /*  Cranberry: We do not need javascript unit testing (qunit)
    'saucelabs-qunit': {
      all: {
        options: {
 ...
 /*   buildcontrol: {
  ...
  if (runSubset('core') &&
    ...
    process.env.TRAVIS_REPO_SLUG !== 'twbs-savage/cranberry') {
